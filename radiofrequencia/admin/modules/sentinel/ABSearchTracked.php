...
$pagetitle = _AB_NUKESENTINEL.": "._AB_SEARCHTRACKIPS;
include("header.php");
title($pagetitle);
OpenMenu();
ipbanmenu();
...
OpenTable();
$tbcol = 6;
$perpage = $abconfig['search_track_perpage'];
if ($perpage == 0) { $perpage = 25; }
if (!isset($showmodule)) $showmodule=" All Modules";
if (!isset($min)) $min=0;
if (!isset($max)) $max=$min+$perpage;
if(!$column or $column=="") $column = $abconfig['search_track_sort_column'];
if(!$direction or $direction=="") $direction = $abconfig['search_track_sort_direction'];
if(ereg("All.*Modules", $showmodule) || !$showmodule ) {
  $modfilter="WHERE ip_addr LIKE '$sip'";
} elseif(ereg("Admin", $showmodule)) {
  $modfilter="WHERE ip_addr LIKE '$sip' AND page LIKE '%admin.php%'";
} elseif(ereg("Index", $showmodule)) {
  $modfilter="WHERE ip_addr LIKE '$sip' AND page LIKE '%index.php%'";
} elseif(ereg("Backend", $showmodule)) {
  $modfilter="WHERE ip_addr LIKE '$sip' AND page LIKE '%backend.php%'";
} else {
  $modfilter="WHERE ip_addr LIKE '$sip' AND page LIKE '%name=$showmodule%'";
}
$totalselected = $db->sql_numrows($db->sql_query("SELECT username, ip_addr, hostname, MAX(date), COUNT(*) FROM ".$prefix."_nsnst_tracked_ips $modfilter GROUP BY 2,3"));
if($totalselected > 0) {
  ...
  echo "<tr>\n";
  // Page Sorting
  echo "<form method='post' action='admin.php?op=ABSearchTracked'>\n";
  ...
  if($column == "c2c,ip_addr") $selcolumn6 = "selected";
  echo "<option value='c2c,ip_addr' $selcolumn6>"._AB_C2CODE."</option>\n";
  echo "</select> ";
  ...
  sort($moduleslist);
  echo "<form action=\"admin.php?op=ABSearchTracked\" method=\"post\">\n";
  echo "<input type='hidden' name='sip' value='$sip'>\n";
  ...
  $result = $db->sql_query("SELECT user_id, username, ip_addr, hostname, MAX(date), COUNT(*), MIN(tid), c2c FROM ".$prefix."_nsnst_tracked_ips $modfilter GROUP BY 3,4 ORDER BY $column $direction LIMIT $min, $perpage");
  while(list($userid,$username,$ipaddr,$hostnm,$lastview,$hits,$tid, $c2c) = $db->sql_fetchrow($result)){
    ...
    echo "<td width='2%'><b><img src='images/sentinel/countries/".$countrytitle['c2c'].".png' height='10' width='16' alt='".$countrytitle['country']."' title='".$countrytitle['country']."'></b></td>\n";
    ...
    echo "href='admin.php?op=ABTrackedDelete&amp;tid=$tid&amp;min=$min&amp;column=$column&amp;direction=$direction&amp;showmodule=$showmodule&amp;xop=$op&amp;sip=$sip'><img src='images/sentinel/delete.png' height='16' width='16' alt='"._AB_DELETE."' title='"._AB_DELETE."' border='0'></a>&nbsp;</td>";
    echo "</tr>";
  }
