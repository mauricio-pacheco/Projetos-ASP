...
$pagetitle = _AB_NUKESENTINEL.": "._AB_SEARCHBLOCKIPS;
include("header.php");
title($pagetitle);
OpenMenu();
ipbanmenu();
...
OpenTable();
$tbcol = 6;
$perpage = $abconfig['search_block_perpage'];
if ($perpage == 0) { $perpage = 25; }
if (!isset($min)) $min=0;
if (!isset($max)) $max=$min+$perpage;
if(!$column or $column=="") $column = $abconfig['search_block_sort_column'];
if(!$direction or $direction=="") $direction = $abconfig['search_block_sort_direction'];
...
if ($totalselected > 0) {
  ...
  echo "<input type='hidden' name='op' value='ABSearchBlocked'>\n";
  ...
  echo "</tr>\n";
  $result = $db->sql_query("SELECT * FROM ".$prefix."_nsnst_blocked_ips WHERE ip_addr LIKE '$sip' ORDER BY $column $direction LIMIT $min,$perpage");
  while ($getIPs = $db->sql_fetchrow($result)) {
    ...
    echo "href='admin.php?op=ABBlockedIPEdit&amp;xIPs=".$getIPs['ip_addr']."&amp;min=$min&amp;column=$column&amp;direction=$direction&amp;xop=$op&amp;sip=$sip'><img src='images/sentinel/edit.png' border='0' alt='"._AB_EDIT."' title='"._AB_EDIT."' height='16' width='16'></a>&nbsp;<a ";
    echo "href='admin.php?op=ABBlockedIPDelete&amp;xIPs=".$getIPs['ip_addr']."&amp;min=$min&amp;column=$column&amp;direction=$direction&amp;xop=$op&amp;sip=$sip'><img src='images/sentinel/delete.png' border='0' alt='"._AB_DELETE."' title='"._AB_DELETE."' height='16' width='16'></a>&nbsp;</td>\n";
    echo "</tr>\n";
  }
  // Page Numbering
