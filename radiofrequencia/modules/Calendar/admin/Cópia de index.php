...
function CalendarEditStory($eid) {
    global $user, $tipath, $bgcolor1, $bgcolor2, $aid, $time24Hour,$useInternationalDates,  $prefix, $calcat1, $calcat2, $calcat3, $calcat4, $calcat5;
    $result = mysql_query("select radminarticle, radminsuper, radminCalendarAdmin from ".$prefix."_authors where aid='$aid'");
    list($radminarticle, $radminsuper, $radminCalendarAdmin) = mysql_fetch_row($result);
    $result2 = mysql_query("select aid from ".$prefix."_events where eid='$eid'");
    list($aaid) = mysql_fetch_row($result2);
    if (($radminarticle == 1) AND ($aaid == $aid) OR ($radminsuper == 1) OR ($radminCalendarAdmin == 1)) {
	include ('header.php');
	GraphicAdmin($hlpfile);
	printJavaScript();
	OpenTable();
...
function CalendarRemoveStory($eid, $ok=0) {
    global $module_name, $aid, $prefix;
    $result = mysql_query("select counter, radminarticle, radminsuper, radminCalendarAdmin from ".$prefix."_authors where aid='$aid'");
    list($counter, $radminarticle, $radminsuper, $radminCalendarAdmin) = mysql_fetch_row($result);
    $result2 = mysql_query("select aid from ".$prefix."_events where eid='$eid'");
    list($aaid) = mysql_fetch_row($result2);
    if (($radminarticle == 1) AND ($aaid == $aid) OR ($radminsuper == 1) OR ($radminCalendarAdmin == 1)) {
	if($ok) {
...
function CalendarChangeStory($eid, $uid, $author, $subject, $hometext, $topic, $day, $month, $year, $endday, $endmonth, $endyear, $startHour, $startMin, $startampm, $endHour, $endMin, $endampm, $alldayevent, $barcolor) {
    global $aid, $module_name,$time24Hour,$prefix;
...
if(mysql_num_rows($result) == 0) {
    ...
    echo "<br>";
OpenTable();
echo "<center><b>"._CALCONFIG."</b></center>
