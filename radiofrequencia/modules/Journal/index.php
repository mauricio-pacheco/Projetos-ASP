...
if (!stristr($_SERVER['SCRIPT_NAME'], "modules.php")) {
    ...
    function last20($bgcolor1, $bgcolor2, $bgcolor3, $username) {
        ...
        echo "</tr>";
        $sql = "SELECT j.id, j.joid, j.nop, j.ldp, j.ltp, j.micro, u.user_id, u.username FROM ".$user_prefix."_journal_stats j, ".$user_prefix."_users u where u.username=j.joid ORDER BY 'ldp' DESC";
    ...
    function all($bgcolor1, $bgcolor2, $bgcolor3, $sitename, $username) {
        ...
        echo ("</tr>");
        $sql = "SELECT j.id, j.joid, j.nop, j.ldp, j.ltp, j.micro, u.user_id FROM ".$user_prefix."_journal_stats j, ".$user_prefix."_users u where u.username=j.joid ORDER BY 'joid'";
        $result = $db->sql_query($sql);
        while ($row = $db->sql_fetchrow($result)) {
            $row[id] = intval($row[id]);
            $row[joid] = check_html($row[joid], nohtml);
