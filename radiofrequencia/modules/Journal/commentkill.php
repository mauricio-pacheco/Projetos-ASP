...
$onwhat = intval($onwhat);
        $sql = "DELETE FROM ".$user_prefix."_journal_comments WHERE cid = '$onwhat'";
$db->sql_query($sql);
        echo "<br>";
        openTable();
        ...
        if (is_user($user)) {
            ...
            startjournal($sitename, $user);
            $onwhat = intval($onwhat);
            $sql = "DELETE FROM ".$user_prefix."_journal_comments WHERE cid = '$onwhat' AND aid = '$username'";
            $db->sql_query($sql);
            echo "<br>";
            openTable();
            echo ("<div align=center>"._COMMENTDELETED."<br><br>");
