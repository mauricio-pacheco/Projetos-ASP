...
set rs = abredb.Execute("SELECT * FROM produtos WHERE fabricante LIKE '%"&palavra(0)&"%' "&tipos&" ORDER by fabricante LIMIT " & inicial & "," & final & "")
...
<%response.end
    ...
    set rs2 = abredb.Execute("SELECT count(fabricante) AS total FROM produtos WHERE fabricante LIKE '%"&palavra(0)&"%' "&tipos&"")
    totalreg = rs2("total")
    rs2.close
    set rs2 = nothing
    %>
    		  				     ...
    		  				     <table BORDER="0" CELLSPACING="1" CELLPADDING="0"><tr><td bgcolor=#bfbfbf>
    							 		<table BORDER="0" CELLSPACING="1" CELLPADDING="3"><tr><td bgcolor=#ffffff>
    										   <form action="comprar.asp" method="post" name=comprar<%=rs("idprod")%>><table align=center width=500 cellspacing="1" cellpadding="1">
    										   		  <tr><td valign=center width=400><font style=font-size:11px;font-family:<%=fonte%>><b><font color=000000><%=rs("nome")%></b><br><br><b>Código:</b> <%=rs("fabricante")%><br><b>Preço:</b><%=strlgMoeda &" "& precito%></td><td valign=center><div align=right><br><input type="hidden" name="intProdID" value="<%= intProdID %>"><input type="hidden" name="intQuant" value=1><%if rs.fields("estoque") = "s" then response.write "<a href=""JavaScript: document.comprar"&rs("idprod")&".submit();""><img src="&dirlingua&"/imagens/comprar_2.gif border=0 alt=""Comprar Produto"" onMouseOut=""window.status='';return true;"" onMouseOver=""window.status='Comprar Produto';return true;""></a>&nbsp;&nbsp;" end if%></div></td></tr>
    										   </table></form>
    								    </table>
    						     </table>
    <%
...
<%else
    ...
    'Pesquisa no banco de dados
    sql = "SELECT * FROM produtos WHERE fabricante LIKE '%"&palavra(0)&"%' "&tipos&" ORDER by fabricante"
    rs.open sql
    if rs.eof or rs.bof then%>
       		  	 <table><tr><td align="left" valign="top">
    			 				<table border="0" cellspacing="4" cellpadding="4" width=590><tr><td><font face="<%=fonte%>" style=font-size:11px;color:000000> <a href=default.asp style=text-decoration:none; onMouseOut="window.status='';return true;" onMouseOver="window.status='<%=strLg4%>';return true;"><b><%=strLg4%></b></a> » <%=strLg42%> <b> <%= pesss %> </b><br><br><table border=0 cellspacing=0 width=100% cellpadding=0><tr><td height=5></td></tr><tr><td height=1 bgcolor=<%=cor3%>></td></tr><tr><td height=5></td></tr></table><%=strLg43%> <b>0</b> <%=strLg44%> :<table border=0 cellspacing=0 width=100% cellpadding=0><tr><td height=5></td></tr><tr><td height=1 bgcolor=<%=cor3%>></td></tr><tr><td height=5></td></tr></table><br><br><br><center><b><%=strLg228%></b></center><br><br><br>
	...
	%>
    		  				     <table BORDER="0" CELLSPACING="1" CELLPADDING="0"><tr><td bgcolor=#bfbfbf>
    							 		<table BORDER="0" CELLSPACING="1" CELLPADDING="3"><tr><td bgcolor=#ffffff>
    										   <form action="comprar.asp" method="post" name=comprar<%=rs("idprod")%>><table align=center width=500 cellspacing="1" cellpadding="1">
    										   		  <tr><td valign=center width=400><font style=font-size:11px;font-family:<%=fonte%>><b><font color=000000><%=rs("nome")%></b><br><br><b>Código:</b> <%=rs("fabricante")%><br><b>Preço:</b> <%=strLgMoeda & " " & precito%></td><td valign=center><div align=right><input type="hidden" name="intProdID" value="<%= intProdID %>"><input type="hidden" name="intQuant" value=1><%if rs.fields("estoque") = "s" then response.write "<a href=""JavaScript: document.comprar"&rs("idprod")&".submit();""><img src="&dirlingua&"/imagens/comprar_2.gif border=0 alt=""Comprar Produto"" onMouseOut=""window.status='';return true;"" onMouseOver=""window.status='Comprar Produto';return true;""></a>&nbsp;&nbsp;" end if%></div></td></tr>
    										   </table></form>
    								    </table>
    						     </table>
    <%
