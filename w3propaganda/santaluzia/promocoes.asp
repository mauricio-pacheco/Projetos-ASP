...
set rs2 = abredb.execute("SELECT count(nome) AS total FROM produtos WHERE promocao='s'")
totalreg = rs2("total")
rs2.close
set rs2 = nothing
strNome = nomes("nome")
...
sql = "SELECT * FROM produtos WHERE promocao='s' ORDER by nome"
rs.open sql
rs.movefirst
somando = 0
if rs.eof or rs.bof then%>
...
<form action="comprar.asp" method="post" name=comprar<%=rs("idprod")%>>
<table align=center width=348 cellspacing="1" cellpadding="1"><tr><td width=80 height=100><img src="produtos/<%=rs("impeq")%>" width="75" border="0"></td>
<td valign=center width=300><font style=font-size:11px;font-family:<%=fonte%>><b><font color=000000><%=rs("nome")%></b><br><br><%if rs("consulta") = "s" then %><br><b>Preço:</b><font color="#808080"> Sob Consulta</font><br><% end if %><% if rs("consulta") = "b" then %><b><%=strLg29%></b>&nbsp;<%=strLgMoeda & " " & precito%> ou <font color=#4074A6 size=2><b><%=rs("peso")%></b>&nbsp;<b>X</b></font>&nbsp;<font color=#E60000><%=strLgMoeda & " " & precitoparcelado%></font><br><font color=#005500><b>Preço Parcelado:</b></font>&nbsp;<font color=#4074A6 size=2><b><%=rs("vezes")%></b>&nbsp;<b>X</b></font>&nbsp;<font color=#E60000>R$ <%=rs("vezes")%></font><br><% end if %><b><%=strLg28%></b> <%if rs("estoque") = "s" then response.write strLg26 else response.write strLg27 end if%>&nbsp;<%=strLg25%><div align=right><br><input type="hidden" name="intProdID" value="<%= intProdID %>"><input type="hidden" name="intQuant" value=1><%if rs.fields("estoque") = "s" then response.write "" end if%>&nbsp;<%if rs.fields("promocao") = "b" then response.write " " & "" else if rs.fields("promocao") = "s" then response.write " " & "<img src='promo.gif'>" else if rs.fields("promocao") = "n" then response.write " " & "<img src='lancamento.gif'>" end if%>&nbsp;<a href="produtos.asp?produto=<%=rs("idprod")%>"  onMouseOut="window.status='';return true;" onMouseOver="window.status='<%=strLg30%>';return true;" style="color:#FFFFFF;text-decoration:none;"><img src=<%=dirlingua%>/imagens/detalhes.gif border="0"></a></div></td></form></tr></table>
</td></tr></table>
</td></tr></table>
