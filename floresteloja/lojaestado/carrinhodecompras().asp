		 ...
		 <script LANGUAGE="JScript">
			...
			function HandleError<%= intProdID %>(eSrc){var val = parseInt(eSrc.value);
				...
				if (val <= 0) {
				 ...
				 <tr><td width="15%" valign="center" align="left"><font face="<%=fonte%>" size="2"> <input name="quant<%= intProdID %>" size="2" value="<%=intQuant%>" onChange="HandleError<%= intProdID %>(this)" style=font-size:11px;font-family:<%=fonte%>; maxlength=4></font></td><td width="39%" align="left"><font face="<%=fonte%>" style=font-size:11px;color:000000><b><%= strProdNome %></b></font></td><td width="16%" align="right"><font face="<%=fonte%>" style=font-size:11px;color:000000><%= strLgMoeda & " " & subpreco %></font></td><td width="20%" align="right"><font face="<%=fonte%>" style=font-size:11px;color:000000><%= strLgMoeda & " " & totpreco %></font></td><td width="10%" align="right"><center><a href="atualizapedido.asp?acao=remover&produto=<%= intProdID %>" onMouseOut="window.status='';return true;" onMouseOver="window.status='<%=strLg62%>';return true;"><img src=<%=dirlingua%>/imagens/del.gif border=0></a></td></tr>
<%pedidos.MoveNext
wend
pedidos.Close
set pedidos = Nothing
